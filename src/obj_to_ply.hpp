...
#ifndef _OBJ_TO_PLY_HPP
#define _OBJ_TO_PLY_HPP
...
void write_file(const char *output);

/******************************************************************************/

int ObjToPly(const char* input, const char *output)
...
*/
    ...
    if (PlyIO::equal_strings(first_word, "v"))
    {
      if (nwords < 4)
      {
    ...
    else if (PlyIO::equal_strings(first_word, "vn"))
    {
    }
    else if (PlyIO::equal_strings(first_word, "vt"))
    {
    }
    else if (PlyIO::equal_strings(first_word, "f"))
    {
...
*/
  ply = PlyIO::write_ply(fp, nelems, elem_names, PLY_ASCII);
  /*
  Describe what properties go into the vertex elements.
*/
  PlyIO::describe_element_ply(ply, "vertex", nverts);
  PlyIO::describe_property_ply(ply, &vert_props[0]);
  PlyIO::describe_property_ply(ply, &vert_props[1]);
  PlyIO::describe_property_ply(ply, &vert_props[2]);

  if (has_normals)
  {
    PlyIO::describe_property_ply(ply, &vert_props[3]);
    PlyIO::describe_property_ply(ply, &vert_props[4]);
    PlyIO::describe_property_ply(ply, &vert_props[5]);
  }

  if (texture_coords)
  {
    PlyIO::describe_property_ply(ply, &vert_props[6]);
    PlyIO::describe_property_ply(ply, &vert_props[7]);
  }

  PlyIO::describe_element_ply(ply, "face", nfaces);
  PlyIO::describe_property_ply(ply, &face_props[0]);
  /*
  Insert the comments.
*/
  for (i = 0; i < ncomments; i++)
  {
    PlyIO::append_comment_ply(ply, comments[i]);
  }
  PlyIO::append_comment_ply(ply, "converted from OBJ by obj2ply");

  PlyIO::header_complete_ply(ply);
  /*
  Set up and write the vertex elements.
*/
  PlyIO::put_element_setup_ply(ply, "vertex");
  for (i = 0; i < nverts; i++)
  {
    PlyIO::put_element_ply(ply, (void *)&vlist[i]);
  }
  /*
  Set up and write the face elements.
*/
  PlyIO::put_element_setup_ply(ply, "face");
  for (i = 0; i < nfaces; i++)
  {
    PlyIO::put_element_ply(ply, (void *)&flist[i]);
  }
